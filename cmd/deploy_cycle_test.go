...
func TestRunDeployCyclePreflightsPluginPullBeforeStoppingSite(t *testing.T) {
	restore := stubDeployCycle(t)
	defer restore()
	...
	want := []string{
		"git",
		"resolve-rollout",
		"rollout:docker compose pull --ignore-buildable",
		"hook:pre-down",
		"compose:down --remove-orphans",
		"rollout:docker compose build --pull,docker compose up -d",
		"hook:post-up",
	}
	if !reflect.DeepEqual(events, want) {
		t.Fatalf("deploy events = %#v, want %#v", events, want)
